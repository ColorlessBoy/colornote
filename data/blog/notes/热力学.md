...
![f41-06|light](/static/notes/f41-06_tc_big.svgz)

![f41-06|dark](/static/notes/f41-06_tc_big_dark.svgz)

花粉在水面上会进行随机游走，爱因斯坦在二十世纪初，对这个运动进行了定量化的描述，非常神奇
。设花粉初始状态为原点，花粉每一次转向为一步，每一步的位移是一个相同分布的随机变
量$\mathbb{L}$。第$N$步，花粉与原点的距离$\mathbf{R}_N$的平方满足如下关系
：$\mathbb{E}[\mathbf{R_N}^2] = \mathbb{E}[(\mathbf{R}_{N-1} + \mathbf{L})^2] = \mathbb{E}[\mathbf{R}^2_{N-1} + 2 \mathbf{R}_{N-1} \mathbf{L} + \mathbf{L}^2]$。
随机游走应该满足马尔可夫性，所以$\mathbb{E}[\mathbf{R}_{N-1} \mathbf{L}]=0$。带入上式可
得
：$\mathbb{E}[\mathbf{R_N}^2] =\mathbb{E}[\mathbf{R}^2_{N-1}] + \mathbb{E}[\mathbf{L}^2]$，
递归可得
...
求解偏微分方程可得：$\mathbb{E}[x^2] = 2kTt/\mu$。因
